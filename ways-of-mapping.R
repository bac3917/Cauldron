...
# Goal: map points on top of county map--> points on choropleth
...
HSlocations <- read.csv("//filesvr01/Research_and_Evaluation_Group/GENERAL/GIS_Shapefiles/CSC_Projects/BerksIU/OCDEL EHS CCP Center Information.csv")
HSlocations<-subset(HSlocations, select=-c(CenterName, Address, Email, City, Zip, HUB, Contact, Phone, MPI) )  #remove unnecessary vars
library(reshape)
HSlocations<-rename(HSlocations,c("LON" = "long", "LAT"="lat") )
head(HSlocations)
...
bb<-qq+geom_point(aes(x=HSlocations$long,y=HSlocations$lat),data=HSlocations)   #THIS WORKS!!!!!
